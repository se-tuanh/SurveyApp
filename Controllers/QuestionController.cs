...
namespace SurveyApp.Controllers
{
    ...
    public class QuestionController : Controller
    {
        ...
        public async Task<IActionResult> Create(Question model, List<string>? optionTexts, int? correctOptionIndex)
        {
            ...
            if (model.QuestionType == QuestionType.MultipleChoice && optionTexts != null)
            {
                ...
                foreach (var text in optionTexts.Where(t => !string.IsNullOrWhiteSpace(t)))
                {
                    ...
                    { 
                        QuestionId = model.Id, 
                        Text = text.Trim(),
                        IsCorrect = correctOptionIndex.HasValue && correctOptionIndex.Value == index
                    });
                    index++;
                }
                await _context.SaveChangesAsync();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, Question model, List<string>? optionTexts, int? correctOptionIndex)
        {
